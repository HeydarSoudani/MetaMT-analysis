...
def macro_f1(array):
    ...
    for i in range(n - 1):
        p[i] = (
            array[i][i] / sum([array[j][i] for j in range(n - 1)])
            if sum([array[j][i] for j in range(n - 1)]) != 0
            else 0
        )
        r[i] = (
            array[i][i] / sum([array[i][j] for j in range(n - 1)])
            if sum([array[i][j] for j in range(n - 1)]) != 0
            else 0
        )
